    ...
    DatabaseService dbService) : ControllerBase
{
    ...
    public async Task<IActionResult> GetPlayerState()
    {
        ...
        return !IsAllowedToPerform(player, user, out var result)
            ? result
            : Ok(JsonSerializer.Serialize(new SocketPlayerDTO(player)));
    }
    ...
    public async Task<IActionResult> GetQueue()
    {
        ...
        return !IsAllowedToPerform(player, user, out var result)
            ? result
            : Ok(JsonSerializer.Serialize(new QueueDTO(player.Queue)));
    }
    ...
    public async Task<IActionResult> GetActions(long? lastActionTimestamp = null)
    {
        ...
        return !IsAllowedToPerform(player, user, out var result)
            ? result
            : Ok(JsonSerializer.Serialize(player.GetActionsForSerialization()));
    }
    ...
    public async Task<IActionResult> GetCurrentTrack()
    {
        ...
        return !IsAllowedToPerform(player, user, out var result)
            ? result
            : Ok(JsonSerializer.Serialize(new TrackDTO(player.CurrentItem)));
    }
    ...
    public async Task<IActionResult> Play([FromQuery] string url)
    {
        ...
        var searchResult = await audioService.Tracks
                                             .LoadTracksAsync(
                                                 url,
                                                 new TrackLoadOptions(TrackSearchMode.None, StrictSearchBehavior.Throw))
        ...
        {
            await player.PlayAsync(user, new ExtendedTrackItem(searchResult.Tracks[0], user), false)
                        .ConfigureAwait(false);
        }
    ...
    public async Task<IActionResult> Add([FromQuery] string url)
    {
        ...
        var track = await audioService.Tracks
                                      .LoadTrackAsync(
                                          url, new TrackLoadOptions(TrackSearchMode.None, StrictSearchBehavior.Throw))
                                      .ConfigureAwait(false);
        if (track is null)
        {
            return NotFound();
    ...
    private bool IsAllowedToPerform([NotNullWhen(true)] SocketPlayer? player,
                                    [NotNullWhen(true)] IUser? user,
                                    [NotNullWhen(false)] out IActionResult? result)
    {
