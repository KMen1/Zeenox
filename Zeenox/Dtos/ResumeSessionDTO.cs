...
public class ResumeSessionDTO(string channelName, TrackDTO currentTrack, int queueLength, List<TrackDTO> queue, long timestamp)
{
        ...
        this(((IVoiceChannel)client.GetChannel(resumeSession.ChannelId)).Name,
            ...
            resumeSession.Queue.Take(5).Select(x =>
                new TrackDTO(new ExtendedTrackItem(LavalinkTrack.Parse(x.Id, null),
                    client.GetUser(x.RequesterId.GetValueOrDefault())))).ToList(), resumeSession.Timestamp) {}
}