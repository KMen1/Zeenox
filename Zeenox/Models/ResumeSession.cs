...
public class ResumeSession(ulong guildId, ulong channelId, TrackStore currentTrackId, List<TrackStore> queue)
{
    ...
    public ResumeSession(LoggedPlayer player) : this(player.GuildId, player.VoiceChannelId, new TrackStore(player.CurrentItem ?? player.LastCurrentItem ?? (ExtendedTrackItem)player.Queue[0]), player.Queue.Select(x => new TrackStore((ExtendedTrackItem)x)).ToList()) { }
}